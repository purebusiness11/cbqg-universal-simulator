...
# Keep as real HTML (not escaped) since this is inside an HTML-enabled markdown call.
st.markdown("""
...
# Appendix B-faithful equations (ε = 1e-9 included where specified). [1](https://discuss.streamlit.io/t/streamlitduplicateelementid-error-with-plotly-charts-in-a-while-true-loop/92083)
EPS = 1e-9
...
def v_eff(v0, chi):
    c = clamp_chi(chi)
    return v0 * (1.0 - c)  # Strictly linear per Appendix B. [1](https://discuss.streamlit.io/t/streamlitduplicateelementid-error-with-plotly-charts-in-a-while-true-loop/92083)
...
""")
    ...
    if view_mode == "3D Spacetime (Gravity Well)":
        ...
        # Native Plotly animation: one Streamlit render, browser runs the frames (prevents Streamlit duplicate element errors). 
        c_targ_init = clamp_chi(chi_global)
        rad_init = univ_R * np.sqrt(max(0.0, 1.0 - c_targ_init**2)) if scale_mode == "Physical (Nonlinear Metric Compression)" else univ_R * (1.1 - c_targ_init)
        st.metric("Hypersphere True Radius", f"{rad_init:.2e} m")
        ...
        fig_life = go.Figure(data=go.Surface(
            x=rad_init * np.cos(u) * np.sin(v),
            y=rad_init * np.sin(u) * np.sin(v),
            z=rad_init * np.cos(v),
            opacity=0.8, colorscale="Plasma", showscale=False
        ))

        anim_frames = []
        for step_val in np.linspace(chi_global, 0.01, 20):
            c_t = clamp_chi(step_val)
            r_frame = univ_R * np.sqrt(max(0.0, 1.0 - c_t**2)) if scale_mode == "Physical (Nonlinear Metric Compression)" else univ_R * (1.1 - c_t)
            anim_frames.append(go.Frame(
                data=[go.Surface(
                    x=r_frame * np.cos(u) * np.sin(v),
                    y=r_frame * np.sin(u) * np.sin(v),
                    z=r_frame * np.cos(v)
                )],
                name=f"{step_val:.4f}"
            ))
        ...
        fig_life.update_layout(
            ...
            updatemenus=[dict(
                ...
                yanchor="bottom",
                buttons=[dict(
                    label="▶ PLAY NATIVE ANIMATION (LIFE CYCLE)",
                    method="animate",
                    args=[None, dict(
        ...
        st.plotly_chart(fig_life, use_container_width=True, key="native_plotly_anim")
...
with t2:
    ...
    with colB:
        ...
        fig2.add_trace(go.Scatter3d(x=tx, y=ty, z=tz,
                                    mode='lines', line=dict(width=6, color=line_color),
                                    name=bridge_name))
...
with t3:
    ...
    st.warning("⚠️ SPECULATIVE ENGINEERING: The following applies CBQG heuristic physics to reverse-engineer reported UAP kinematics. Math core stays Appendix-B. [1](https://discuss.streamlit.io/t/streamlitduplicateelementid-error-with-plotly-charts-in-a-while-true-loop/92083)")

    st.markdown("""
When radar systems track anomalous craft, they report kinematics irreconcilable with General Relativity. Under CBQG, if a craft accumulates localized spacetime geometrically tight to its hull limit (χ → 1), **acceleration increases without bound under finite force (via m_eff → 0)**.
""")
    ...
    # shared scalars (used both in KPI section and the dynamic plot)
    ...
    fig3.add_trace(go.Scatter(
        x=time_arr,
        y=[base_accel] * len(time_arr),
        name=f"Conventional Apparent Accel ({base_accel:,.1f} m/s²)",
    ...
    fig3.update_layout(
        ...
        margin=dict(l=0, r=0, b=0, t=40),
        xaxis_title="Seconds",
        yaxis_title="Apparent Kinematics (m/s²)",
        yaxis_type="log"
    )
